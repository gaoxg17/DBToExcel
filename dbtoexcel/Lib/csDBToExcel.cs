...
namespace DBToExcel.Lib
{
    ...
    public class csDBToExcel
    {
        ...
        /// <returns></returns>
        public csDBToExcel FromXml(string Xml)
        {
            ...
            try
            {
                ...
                obj = (csDBToExcel)ser.Deserialize(xmlReader);
            }
            catch (Exception ex)
            {
        ...
        public csDBToExcel fromXmlFile(string xmlFileName)
        {
            Stream reader = null;
            csDBToExcel obj = new csDBToExcel();
            try
            {
                XmlSerializer serializer = new XmlSerializer(typeof(csDBToExcel));
                reader = new FileStream(xmlFileName, FileMode.Open);
                obj = (csDBToExcel)serializer.Deserialize(reader);
                reader.Close();
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns>反序列化的对象，失败则返回null</returns>
        public csDBToExcel fromXmlFile()
        {
